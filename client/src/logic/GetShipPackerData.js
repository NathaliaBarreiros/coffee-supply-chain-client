...
const GetShipPackerData = () => {
	const [shipPackerInfo, setShipPackerInfo] = useState({
		transportTypeP: "",
		pickupDateP: "",
		shipPriceP: "",
	});
	...
	const askShipPacker = async (values) => {
		...
		try {
			...
			console.log(info);
			setShipPackerInfo({
				transportTypeP: info["transportTypeP"],
				pickupDateP: info["pickupDateP"],
				shipPriceP: info["shipPriceP"],
			});
		} catch (error) {
...
export default GetShipPackerData;
