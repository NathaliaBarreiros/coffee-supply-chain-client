...
const GetShipRetailerData = () => {
	const [shipRetailerInfo, setShipRetailerInfo] = useState({
		transportTypeR: "",
		pickupDateR: "",
		shipPriceR: "",
	});
	...
	const askShipRetailer = async (values) => {
		...
		try {
			...
			console.log(info);
			setShipRetailerInfo({
				transportTypeP: info["transportTypeR"],
				pickupDateP: info["pickupDateR"],
				shipPriceP: info["shipPriceR"],
			});
		} catch (error) {
...
export default GetShipRetailerData;
